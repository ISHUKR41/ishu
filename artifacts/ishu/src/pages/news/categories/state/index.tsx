// FILE: artifacts/ishu/src/pages/news/categories/state/index.tsx
...
export default function NewsState() {
  const { data, isLoading } = useListNews({ category: "state", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="State News - ISHU | Indian Student Hub"
        description="Latest State news and updates for Indian students and job seekers on ISHU platform."
        keywords="state news, State updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/state`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">State News</h1>
          <p className="text-muted-foreground mb-8">Latest State updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No State news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
