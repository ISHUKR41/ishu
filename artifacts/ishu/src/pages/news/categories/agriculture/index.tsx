// FILE: artifacts/ishu/src/pages/news/categories/agriculture/index.tsx
...
export default function NewsAgriculture() {
  const { data, isLoading } = useListNews({ category: "agriculture", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Agriculture News - ISHU | Indian Student Hub"
        description="Latest Agriculture news and updates for Indian students and job seekers on ISHU platform."
        keywords="agriculture news, Agriculture updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/agriculture`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Agriculture News</h1>
          <p className="text-muted-foreground mb-8">Latest Agriculture updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Agriculture news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
