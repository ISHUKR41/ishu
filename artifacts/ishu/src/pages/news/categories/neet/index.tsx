// FILE: artifacts/ishu/src/pages/news/categories/neet/index.tsx
...
export default function NewsNeet() {
  const { data, isLoading } = useListNews({ category: "neet", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Neet News - ISHU | Indian Student Hub"
        description="Latest Neet news and updates for Indian students and job seekers on ISHU platform."
        keywords="neet news, Neet updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/neet`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Neet News</h1>
          <p className="text-muted-foreground mb-8">Latest Neet updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Neet news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
