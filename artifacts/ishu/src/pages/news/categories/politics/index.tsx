// FILE: artifacts/ishu/src/pages/news/categories/politics/index.tsx
...
export default function NewsPolitics() {
  const { data, isLoading } = useListNews({ category: "politics", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Politics News - ISHU | Indian Student Hub"
        description="Latest Politics news and updates for Indian students and job seekers on ISHU platform."
        keywords="politics news, Politics updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/politics`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Politics News</h1>
          <p className="text-muted-foreground mb-8">Latest Politics updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Politics news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
