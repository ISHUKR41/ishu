// FILE: artifacts/ishu/src/pages/news/categories/startups/index.tsx
...
export default function NewsStartups() {
  const { data, isLoading } = useListNews({ category: "startups", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Startups News - ISHU | Indian Student Hub"
        description="Latest Startups news and updates for Indian students and job seekers on ISHU platform."
        keywords="startups news, Startups updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/startups`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Startups News</h1>
          <p className="text-muted-foreground mb-8">Latest Startups updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Startups news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
