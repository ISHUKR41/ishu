...
export default function NewsTeaching() {
  const { data, isLoading } = useListNews({ category: "teaching", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Teaching News - ISHU | Indian Student Hub"
        description="Latest Teaching news and updates for Indian students and job seekers on ISHU platform."
        keywords="teaching news, Teaching updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/teaching`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Teaching News</h1>
          <p className="text-muted-foreground mb-8">Latest Teaching updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Teaching news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
