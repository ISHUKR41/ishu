// FILE: artifacts/ishu/src/pages/news/categories/police/index.tsx
...
export default function NewsPolice() {
  const { data, isLoading } = useListNews({ category: "police", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Police News - ISHU | Indian Student Hub"
        description="Latest Police news and updates for Indian students and job seekers on ISHU platform."
        keywords="police news, Police updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/police`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Police News</h1>
          <p className="text-muted-foreground mb-8">Latest Police updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Police news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
