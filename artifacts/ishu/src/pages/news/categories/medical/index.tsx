// FILE: artifacts/ishu/src/pages/news/categories/medical/index.tsx
...
export default function NewsMedical() {
  const { data, isLoading } = useListNews({ category: "medical", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Medical News - ISHU | Indian Student Hub"
        description="Latest Medical news and updates for Indian students and job seekers on ISHU platform."
        keywords="medical news, Medical updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/medical`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Medical News</h1>
          <p className="text-muted-foreground mb-8">Latest Medical updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Medical news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
