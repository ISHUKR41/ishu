// FILE: artifacts/ishu/src/pages/news/categories/health/index.tsx
...
export default function NewsHealth() {
  const { data, isLoading } = useListNews({ category: "health", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Health News - ISHU | Indian Student Hub"
        description="Latest Health news and updates for Indian students and job seekers on ISHU platform."
        keywords="health news, Health updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/health`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Health News</h1>
          <p className="text-muted-foreground mb-8">Latest Health updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Health news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
