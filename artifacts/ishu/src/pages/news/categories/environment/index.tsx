// FILE: artifacts/ishu/src/pages/news/categories/environment/index.tsx
...
export default function NewsEnvironment() {
  const { data, isLoading } = useListNews({ category: "environment", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Environment News - ISHU | Indian Student Hub"
        description="Latest Environment news and updates for Indian students and job seekers on ISHU platform."
        keywords="environment news, Environment updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/environment`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Environment News</h1>
          <p className="text-muted-foreground mb-8">Latest Environment updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Environment news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
