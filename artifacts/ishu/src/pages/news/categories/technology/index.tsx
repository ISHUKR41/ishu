// FILE: artifacts/ishu/src/pages/news/categories/technology/index.tsx
...
export default function NewsTechnology() {
  const { data, isLoading } = useListNews({ category: "technology", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Technology News - ISHU | Indian Student Hub"
        description="Latest Technology news and updates for Indian students and job seekers on ISHU platform."
        keywords="technology news, Technology updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/technology`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <p className="text-muted-foreground mb-8">Latest Technology updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Technology news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
