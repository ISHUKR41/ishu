// FILE: artifacts/ishu/src/pages/news/categories/education/index.tsx
...
export default function NewsEducation() {
  const { data, isLoading } = useListNews({ category: "education", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Education News - ISHU | Indian Student Hub"
        description="Latest Education news and updates for Indian students and job seekers on ISHU platform."
        keywords="education news, Education updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/education`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Education News</h1>
          <p className="text-muted-foreground mb-8">Latest Education updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Education news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
