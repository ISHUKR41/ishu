// FILE: artifacts/ishu/src/pages/news/categories/legal/index.tsx
...
export default function NewsLegal() {
  const { data, isLoading } = useListNews({ category: "legal", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Legal News - ISHU | Indian Student Hub"
        description="Latest Legal news and updates for Indian students and job seekers on ISHU platform."
        keywords="legal news, Legal updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/legal`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Legal News</h1>
          <p className="text-muted-foreground mb-8">Latest Legal updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Legal news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
