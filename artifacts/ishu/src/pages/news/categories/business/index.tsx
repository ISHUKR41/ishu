// FILE: artifacts/ishu/src/pages/news/categories/business/index.tsx
...
export default function NewsBusiness() {
  const { data, isLoading } = useListNews({ category: "business", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Business News - ISHU | Indian Student Hub"
        description="Latest Business news and updates for Indian students and job seekers on ISHU platform."
        keywords="business news, Business updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/business`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Business News</h1>
          <p className="text-muted-foreground mb-8">Latest Business updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Business news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
