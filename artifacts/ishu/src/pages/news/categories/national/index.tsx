// FILE: artifacts/ishu/src/pages/news/categories/national/index.tsx
...
export default function NewsNational() {
  const { data, isLoading } = useListNews({ category: "national", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="National News - ISHU | Indian Student Hub"
        description="Latest National news and updates for Indian students and job seekers on ISHU platform."
        keywords="national news, National updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/national`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">National News</h1>
          <p className="text-muted-foreground mb-8">Latest National updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No National news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
