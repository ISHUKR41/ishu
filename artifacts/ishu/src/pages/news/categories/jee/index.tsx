// FILE: artifacts/ishu/src/pages/news/categories/jee/index.tsx
...
export default function NewsJee() {
  const { data, isLoading } = useListNews({ category: "jee", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Jee News - ISHU | Indian Student Hub"
        description="Latest Jee news and updates for Indian students and job seekers on ISHU platform."
        keywords="jee news, Jee updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/jee`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Jee News</h1>
          <p className="text-muted-foreground mb-8">Latest Jee updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Jee news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
