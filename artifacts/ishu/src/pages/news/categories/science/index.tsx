// FILE: artifacts/ishu/src/pages/news/categories/science/index.tsx
...
export default function NewsScience() {
  const { data, isLoading } = useListNews({ category: "science", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Science News - ISHU | Indian Student Hub"
        description="Latest Science news and updates for Indian students and job seekers on ISHU platform."
        keywords="science news, Science updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/science`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Science News</h1>
          <p className="text-muted-foreground mb-8">Latest Science updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Science news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
