// FILE: artifacts/ishu/src/pages/news/categories/innovation/index.tsx
...
export default function NewsInnovation() {
  const { data, isLoading } = useListNews({ category: "innovation", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Innovation News - ISHU | Indian Student Hub"
        description="Latest Innovation news and updates for Indian students and job seekers on ISHU platform."
        keywords="innovation news, Innovation updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/innovation`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Innovation News</h1>
          <p className="text-muted-foreground mb-8">Latest Innovation updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Innovation news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
