// FILE: artifacts/ishu/src/pages/news/categories/engineering/index.tsx
...
export default function NewsEngineering() {
  const { data, isLoading } = useListNews({ category: "engineering", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Engineering News - ISHU | Indian Student Hub"
        description="Latest Engineering news and updates for Indian students and job seekers on ISHU platform."
        keywords="engineering news, Engineering updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/engineering`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Engineering News</h1>
          <p className="text-muted-foreground mb-8">Latest Engineering updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Engineering news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
