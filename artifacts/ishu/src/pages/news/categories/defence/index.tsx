// FILE: artifacts/ishu/src/pages/news/categories/defence/index.tsx
...
export default function NewsDefence() {
  const { data, isLoading } = useListNews({ category: "defence", limit: 50 });
  const newsList = (data as any)?.news ?? data ?? [];

  return (
    <>
      <PageMeta
        title="Defence News - ISHU | Indian Student Hub"
        description="Latest Defence news and updates for Indian students and job seekers on ISHU platform."
        keywords="defence news, Defence updates, indian education, ishu"
        canonical={`https://ishu.in/news/category/defence`}
      />
      <div className="min-h-screen py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold mb-2">Defence News</h1>
          <p className="text-muted-foreground mb-8">Latest Defence updates and announcements</p>
          {isLoading ? (
            ...
            <div className="text-center py-20 text-muted-foreground">
              No Defence news available yet. Check back soon.
            </div>
          )}
        </div>
      </div>
