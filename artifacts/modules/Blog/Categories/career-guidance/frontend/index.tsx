// ============================================================================
// FILE: modules/Blog/Categories/career-guidance/frontend/index.tsx
// PURPOSE: Isolated frontend for the "Career Guidance" blog category.
...
export default function CareerguidanceBlogPage() {
  const containerRef = useRef<HTMLDivElement>(null);

  const { data, isLoading, error } = useQuery<{ articles: BlogArticle[]; total: number }>({
    queryKey: ["blogs", "category", "career-guidance"],
    queryFn: async () => {
      const res = await fetch("/api/blogs/category/career-guidance");
      if (!res.ok) throw new Error("Failed to fetch Career Guidance blogs");
      return res.json();
    },
    staleTime: 1000 * 60 * 5,
  });
  ...
  return (
    <>
      <PageMeta
        ...
        description={`Read the best Career Guidance articles, guides, and insights for exam preparation and career growth.`}
      />
        ...
        <div className="border-b border-border bg-gradient-to-b from-green-500/5 to-background py-10">
          <div className="container mx-auto px-4 md:px-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-foreground">Career Guidance</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.articles?.length > 0 ? (
                ...
                <div className="col-span-full text-center py-16 border border-dashed border-border rounded-2xl">
                  ...
                  <p className="text-muted-foreground">No Career Guidance articles yet.</p>
                </div>
              )}
            </div>
          )}
