// ============================================================================
// FILE: modules/Blog/Categories/success-stories/frontend/index.tsx
// PURPOSE: Isolated frontend for the "Success Stories" blog category.
...
export default function SuccessstoriesBlogPage() {
  const containerRef = useRef<HTMLDivElement>(null);

  const { data, isLoading, error } = useQuery<{ articles: BlogArticle[]; total: number }>({
    queryKey: ["blogs", "category", "success-stories"],
    queryFn: async () => {
      const res = await fetch("/api/blogs/category/success-stories");
      if (!res.ok) throw new Error("Failed to fetch Success Stories blogs");
      return res.json();
    },
    staleTime: 1000 * 60 * 5,
  });
  ...
  return (
    <>
      <PageMeta
        ...
        description={`Read the best Success Stories articles, guides, and insights for exam preparation and career growth.`}
      />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.articles?.length > 0 ? (
                ...
                <div className="col-span-full text-center py-16 border border-dashed border-border rounded-2xl">
                  ...
                  <p className="text-muted-foreground">No Success Stories articles yet.</p>
                </div>
              )}
            </div>
          )}
