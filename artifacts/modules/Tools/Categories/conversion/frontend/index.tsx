// ============================================================================
// FILE: modules/Tools/Categories/conversion/frontend/index.tsx
// PURPOSE: Isolated frontend for "Conversion Tools" tools category.
...
export default function ConversionToolsPage() {
  ...
  const { data, isLoading, error } = useQuery<{ tools: ToolItem[]; total: number }>({
    queryKey: ["tools", "category", "conversion"],
    queryFn: async () => {
      const res = await fetch("/api/tools/category/conversion");
      if (!res.ok) throw new Error("Failed to fetch Conversion Tools");
      return res.json();
    },
    staleTime: 1000 * 60 * 5,
  });
  ...
  return (
    <>
      <PageMeta
        ...
        description={`Use free online Conversion Tools — fast, secure, and no registration required.`}
      />
        ...
        <div className="border-b border-border bg-gradient-to-b from-cyan-500/5 to-background py-10">
          <div className="container mx-auto px-4 md:px-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-foreground">Conversion Tools</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.tools?.length > 0 ? (
                ...
                <div className="col-span-full text-center py-16 border border-dashed border-border rounded-2xl">
                  ...
                  <p className="text-muted-foreground">No Conversion Tools available yet.</p>
                </div>
              )}
            </div>
          )}
