// ============================================================================
// FILE: modules/Resources/Categories/formula-sheets/frontend/index.tsx
// PURPOSE: Isolated frontend for "Formula Sheets" resources category.
// ============================================================================
...
export default function FormulasheetsResourcesPage() {
  const containerRef = useRef<HTMLDivElement>(null);

  const { data, isLoading, error } = useQuery<{ resources: ResourceItem[] }>({
    queryKey: ["resources", "category", "formula-sheets"],
    queryFn: async () => {
      const res = await fetch("/api/resources/category/formula-sheets");
      if (!res.ok) throw new Error("Failed to fetch Formula Sheets");
      return res.json();
    },
    staleTime: 1000 * 60 * 5,
  });
        ...
        <div className="border-b border-border bg-gradient-to-b from-amber-500/5 to-background py-10">
          <div className="container mx-auto px-4 md:px-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-foreground">Formula Sheets</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.resources?.length > 0 ? (
                ...
                <div className="col-span-full text-center py-16 border border-dashed border-border rounded-2xl">
                  ...
                  <p className="text-muted-foreground">No Formula Sheets available yet.</p>
                </div>
              )}
            </div>
          )}
