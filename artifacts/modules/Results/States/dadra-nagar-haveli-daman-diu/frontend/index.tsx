// ============================================================================
// FILE: modules/Results/States/dadra-nagar-haveli-daman-diu/frontend/index.tsx
// PURPOSE: Isolated frontend for Dadra Nagar Haveli & Daman Diu (DN) state results.
...
/**
 * Dadra Nagar Haveli & Daman DiuResultsPage — The dedicated page for Dadra Nagar Haveli & Daman Diu exam results.
 *
 * WHAT IT DOES:
 * 1. Fetches results filtered by state="Dadra Nagar Haveli & Daman Diu" from the backend API.
 ...
 */
export default function DadranagarhavelidamandiuResultsPage() {
  ...
  const { data, isLoading, error } = useQuery<{ results: StateResult[]; total: number }>({
    queryKey: ["results", "state", "dadra-nagar-haveli-daman-diu"],
    queryFn: async () => {
      const res = await fetch("/api/results?state=Dadra Nagar Haveli & Daman Diu&limit=50");
      if (!res.ok) throw new Error("Failed to fetch Dadra Nagar Haveli & Daman Diu results");
      return res.json();
    },
  ...
  return (
    <>
      ...
      <PageMeta
        ...
        description={`Get the latest Dadra Nagar Haveli & Daman Diu government exam results, notifications, and updates. Stay informed about all state-level examinations.`}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-foreground">
                  Dadra Nagar Haveli & Daman Diu Results
                </h1>
                <p className="text-muted-foreground text-sm">
                  State Code: DN • All government exam results
                </p>
              </div>
            </div>
        ...
        <div className="container mx-auto px-4 md:px-6 py-10">
          ...
          {isLoading && (
            <div className="flex justify-center items-center py-20">
              ...
              <span className="ml-3 text-muted-foreground">Loading Dadra Nagar Haveli & Daman Diu results...</span>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.results.length > 0 ? (
                ...
                <div className="col-span-full text-center py-16 border border-dashed border-border rounded-2xl">
                  ...
                  <p className="text-muted-foreground">No results found for Dadra Nagar Haveli & Daman Diu yet.</p>
