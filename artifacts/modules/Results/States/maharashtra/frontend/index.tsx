// ============================================================================
// FILE: modules/Results/States/maharashtra/frontend/index.tsx
// PURPOSE: Isolated frontend for Maharashtra (MH) state results.
...
/**
 * MaharashtraResultsPage — The dedicated page for Maharashtra exam results.
 *
 * WHAT IT DOES:
 * 1. Fetches results filtered by state="Maharashtra" from the backend API.
...
export default function MaharashtraResultsPage() {
  ...
  const { data, isLoading, error } = useQuery<{ results: StateResult[]; total: number }>({
    queryKey: ["results", "state", "maharashtra"],
    queryFn: async () => {
      const res = await fetch("/api/results?state=Maharashtra&limit=50");
      if (!res.ok) throw new Error("Failed to fetch Maharashtra results");
      return res.json();
    },
  ...
  return (
    <>
      ...
      <PageMeta
        ...
        description={`Get the latest Maharashtra government exam results, notifications, and updates. Stay informed about all state-level examinations.`}
        keywords="Maharashtra results, Maharashtra exam results, Maharashtra government jobs, MH results"
      />
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  State Code: MH • All government exam results
                </p>
              </div>
            </div>
        ...
        <div className="container mx-auto px-4 md:px-6 py-10">
          ...
          {isLoading && (
            <div className="flex justify-center items-center py-20">
              ...
              <span className="ml-3 text-muted-foreground">Loading Maharashtra results...</span>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.results.length > 0 ? (
                ...
                <div className="col-span-full text-center py-16 border border-dashed border-border rounded-2xl">
                  ...
                  <p className="text-muted-foreground">No results found for Maharashtra yet.</p>
