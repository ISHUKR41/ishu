...
TEMP_DIR = Path(tempfile.gettempdir()) / "ishu_tools" / "unlock"
...
async def unlock_pdf(file: UploadFile = File(...), password: Optional[str] = Form(None)):
    ...
    try:
        ...
        if reader.is_encrypted:
            unlock_status = reader.decrypt(password or "")
            if unlock_status == 0:
                raise HTTPException(status_code=400, detail="Unable to unlock PDF. Invalid password.")

        writer = PdfWriter()
        for page in reader.pages:
            writer.add_page(page)

        output_path = work_dir / "unlocked.pdf"
